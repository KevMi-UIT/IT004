...
ALTER TABLE KHACHHANG ALTER COLUMN LOAIKH NVARCHAR(10)

--6. Đơn vị tính của sản phẩm chỉ có thể là (“cay”, ”hop”, ”cai”, ”quyen”, ”chuc”)
ALTER TABLE SANPHAM 
ADD CONSTRAINT CHK_DVT
CHECK(DVT = 'cay' OR DVT = 'hop' OR DVT = 'cai' OR DVT = 'chuc' OR DVT ='quyen')
...
ADD CONSTRAINT CHK_NGSINH_NGDK
...
/*12. Cập nhật giá trị LOAIKH là “Vip” đối với những khách hàng đăng ký thành
viên trước ngày 1/1/2007 có doanh số từ 10.000.000 trở lên hoặc khách hàng
đăng ký thành viên từ 1/1/2007 trở về sau có doanh số từ 2.000.000 trở lên
(cho quan hệ KHACHHANG1)
*/
SELECT *
FROM KHACHHANG1
WHERE DATEDIFF(DAY, NGDK, '1/1/2007') > 0 AND DOANHSO >= 10000000 OR
	DATEDIFF(DAY, NGDK, '1/1/2007') <= 0 AND DOANHSO >= 2000000
...
SELECT *
FROM HOADON
WHERE DATEDIFF(DAY, '1/1/2007', NGHD)  >= 0 AND DATEDIFF(DAY, '2/1/2007', NGHD) <= 0
...
SELECT SOHD, TRIGIA
FROM HOADON
WHERE DATEDIFF(DAY, '1/1/2007', NGHD) >= 0 AND DATEDIFF(DAY, '31/1/2007', NGHD) <= 0
...
WHERE KH.MAKH = HD.MAKH AND DATEDIFF(DAY, '1/1/2007', NGHD) = 0
...
WHERE HD.MANV = NV.MANV AND DATEDIFF(DAY, CONVERT(DATETIME, '10/28/2006',101), CONVERT(DATETIME, NGHD,101)) = 0 AND NV.HOTEN = 'Nguyen Van B'
...
SELECT *
FROM SANPHAM SP,
	(SELECT HD.SOHD, HD.MAKH, CT.MASP, NGHD
	FROM HOADON HD, CTHD CT
	WHERE HD.SOHD = CT.SOHD) AS HDCT,
	KHACHHANG KH
WHERE SP.MASP = HDCT.MASP AND HDCT.MAKH = KH.MAKH
	AND KH.HOTEN = 'Nguyen Van A'
	AND MONTH(HDCT.NGHD) = '10' AND YEAR(HDCT.NGHD) = '2006'
